...
{ 	
	if ( !IsErrorRspInfo(pRspInfo) &&  pInstrument){
		...
		string strTemp(pInstrument->InstrumentID);
		if( (strTemp.find("&")!=string::npos)			// 多腿行情
			||(strTemp.find("-P-")!=string::npos)		// 豆粕期权
			||(strTemp.find("-C-")!=string::npos)		// 豆粕期权
			||(strTemp.find("SR")!=string::npos)&&(strTemp.find("C")!=string::npos)
			||(strTemp.find("SR")!=string::npos)&&(strTemp.find("P")!=string::npos)
			||(strTemp.find("efp")!=string::npos)		// 豆粕期权
			)
		{
		}
		else
		{
			...
			{
				strNeedQuote.append(",");
				strNeedQuote.append(strTemp);
			}

			ofstream outfile;
			outfile.open("currentInstrument.txt", ios::app);
...
///请求查询行情
void CtpTraderSpi::ReqQryDepthMarketData(CThostFtdcQryDepthMarketDataField *pQryDepthMarketData)
{
	int ret = pUserApi->ReqQryDepthMarketData(pQryDepthMarketData, ++requestId);
	cerr<<" 请求 | 查询行情..."<<((ret == 0)?"成功":"失败")<<endl;
}
